...
* EXPERIMENTAL. Now in the "Info" tab you can configure the port on which the server will work. This could potentially allow the program and controllers to work on online servers with other players. Or maybe not. That's why this is experimental - it won't work, I'll remove it in the next update. If you also need to configure the port in the controller with which you are using the program, the port is configured at the beginning of the lua code of each controller.
